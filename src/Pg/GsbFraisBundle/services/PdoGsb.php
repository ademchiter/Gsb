...
*/
    public function getInfosVisiteur($login, $mdp){
             $req = "SELECT Visiteur.id as id, Visiteur.nom as nom, Visiteur.prenom as prenom, Visiteur.statut as statut, affectation.idnumImmat as idnumImmat, Visiteur.refTablette as refTablette, tablette.memoire as memoire
from Visiteur, affectation, tablette
where  Visiteur.id = affectation.idVisiteur and 
        Visiteur.login='$login' and Visiteur.mdp='$mdp' and
        Visiteur.refTablette=tablette.id
...
*/ 
    ...
    */
    ...
    public function recupererImmat($idVisiteur){
        $req ="SELECT affectation.idnumImmat as idnumImmat,affectation.dateDebut as dateDebut
        FROM affectation
        WHERE affectation.idVisiteur='$idVisiteur'
        ORDER BY affectation.dateDebut DESC ";
        $res =PdoGsb::$monPdo->query($req);
        $tabImmat = $res->fetch();                                  
        return $tabImmat;
    }
       ...
       public function getListeVehicules($login, $mdp){
                $req = "select numImmat, marque, modele, couleur, dateDebut, dateFin, nom, prenom from affectation, vehicule, Visiteur where Visiteur.id = affectation.idVisiteur and affectation.idnumImmat=vehicule.numImmat";
        //var_dump($req);
        $stmt = PdoGsb::$monPdo->prepare($req);
                $stmt->bindParam(':log', $login);
        $stmt->bindParam(':md', $mdp);
        ...
        public function getMesVehicules($login, $mdp, $id){
                $req = "select nom, numImmat, marque, modele, couleur, dateDebut, dateFin from affectation, vehicule, Visiteur where Visiteur.id = affectation.idVisiteur and affectation.idnumImmat=vehicule.numImmat and Visiteur.id='".$id."'";
        //var_dump($req);
        $stmt = PdoGsb::$monPdo->prepare($req);
                $stmt->bindParam(':log', $login);
        $stmt->bindParam(':md', $mdp);
        ...
        public function addVoiture($numImmat, $marque, $modele, $couleur){
        $requestuest = "INSERT INTO `gsbphp`.`vehicule` (`numImmat`, `marque`, `modele`, `couleur`) VALUES ('".$numImmat."', '".$marque."', '".$modele."', '".$couleur."')";
        $q = PdoGsb::$monPdo->prepare($requestuest);
        $q->execute();
        echo "donnée enregistrer avec succes ";
    }

        public function getVehicules($login, $mdp){
        ...
        return $listevehicules;
            ...
            public function supprimerVehicules($login, $mdp, $idvehicule){
                $req = "DELETE FROM `gsbphp`.`vehicule` where numImmat= $idvehicule";
        //var_dump($req);
        $stmt = PdoGsb::$monPdo->prepare($req);
                $stmt->bindParam(':log', $login);
        $stmt->bindParam(':md', $mdp);
